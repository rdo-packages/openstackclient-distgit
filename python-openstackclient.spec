...
# Generate i18n files
%{__python%{pyver}} setup.py compile_catalog -d build/lib/%{sname}/locale

%install
%{pyver_install}
mv %{buildroot}%{_bindir}/%{cname} %{buildroot}%{_bindir}/%{cname}-%{python%{pyver}_version}
ln -s ./%{cname}-%{python3_version} %{buildroot}%{_bindir}/%{cname}-%{pyver}

%{__python%{pyver}} setup.py install -O1 --skip-build --root %{buildroot}
ln -s ./%{cname} %{buildroot}%{_bindir}/%{cname}-%{pyver}
ln -s ./%{cname} %{buildroot}%{_bindir}/%{cname}-%{python%{pyver}_version}
...
%{_bindir}/%{cname}
%{_bindir}/%{cname}-%{pyver}
%{_bindir}/%{cname}-%{python%{pyver}_version}
%{pyver_sitelib}/%{sname}
